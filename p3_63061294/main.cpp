...
void fifo_schedule(void)
{
    ...
    {
		if( !occupied) {
			int myThread = readyQue[0];
			pthread_cond_signal(&(cond[myThread]));
			readyQue.erase(readyQue.begin()+0);
		}else{
			//printf("\nFIFO: Occupied\n"); 
		}
    }else{
		//printf("\nFIFO: Ready Queue Empty\n"); 
	}
    pthread_mutex_unlock(&mutex);
}
...
void edf_schedule(void)
{
    ...
    if(readyQue.size() > 0){
		if(!occupied){
			int myThread = readyQue[0] ; 
			int index = 0; 
			int lower_threshold = tcb[readyQue[0]].deadline;
			for(int i = 0; i < readyQue.size(); i++){
				if(tcb[readyQue[i]].deadline < lower_threshold){
					myThread = tcb[readyQue[i]].id;
					index = i;
					lower_threshold = tcb[readyQue[i]].deadline;
				}
			}    
			pthread_cond_signal(&(cond[myThread]));
			//Remove From Queue
			readyQue.erase(readyQue.begin()+index);
		}else{
			//printf("\nEDF: Occupied\n"); 
		}
	}else{
		//printf("\nEDF: Ready Queue Empty\n"); 
	}
...
void rm_schedule(void)
{
    ...
    if(readyQue.size() > 0)
    {
        if(!occupied){
			int myThread = readyQue[0]; 
			int index = 0;
			int lower_threshold = tcb[readyQue[0]].period;
			for(int i = 0; i < readyQue.size(); i++)
			{
				if(tcb[readyQue[i]].period < lower_threshold)
				{
					myThread = tcb[readyQue[i]].id;
					index = i;
				}
				lower_threshold = tcb[readyQue[i]].period;
			}
			pthread_cond_signal(&(cond[myThread]));
